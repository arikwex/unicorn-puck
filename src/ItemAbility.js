...
// than keyed, since object keys survive minification. Every icon is drawn
// inline here rather than as its own named function: each was referenced
// exactly once, and the wrappers cost bytes.
const ITEM_ABILITY_CATALOG = [
  ...
  ['BATTLE ARMOR', '+2 max health', (context) => {
    context.beginPath();
  ...
  ['VALKYRIE WINGS', 'increase boost speed', (context) => {
    context.save();
  ...
  ['CHROMATIC HOOF', 'chain lightning', (context) => {
    context.beginPath();
  ...
  ['ORACLE EYES', 'show minimap', (context) => {
    context.beginPath();
    context.moveTo(-16, 0);

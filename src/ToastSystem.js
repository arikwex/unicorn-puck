...
// ToastSystem HUD instance. A new run needs no reset: createMap() shows its
// intro toast right after adding the HUD, replacing any leftover message. Replaces the old event-bus emit('toast', ...)/
...
function ToastSystem() {
  return {
    ...
    // Shown for DURATION seconds after each showToast().
    hud(context) {
      if (!message || elapsed >= DURATION) return;

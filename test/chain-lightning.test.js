...
test('the bolt arcs cleanly in y and ripples along x to the midpoint of each hop after 100 ms', () => {
  ...
  assert.ok(midpointRipple > 0 && midpointRipple <= 10, 'ripple rides x, bounded by the parabola envelope');
  ...
  assert.ok(Math.abs(bolt.path.at(-1)[0] - 175) <= 10, 'same ripple bound on the second hop');
...
test('the animated tail follows the entire path to the final target before disappearing', () => {
  ...
  assert.ok(Math.abs(bolts()[0].path[0][0] - 80) <= 6.4, 'tail follows the head by 240 ms');
  assert.ok(Math.abs(bolts()[0].path[0][1] + 87.6) < 1e-6, 'ripple-free y pins the tail at u = 0.8');
  ...
  assert.ok(Math.abs(bolts()[0].path[0][0] - 90) <= 3.6, 'tail advances along the first hop');
  assert.ok(Math.abs(bolts()[0].path[0][1] + 62.4) < 1e-6, 'ripple-free y pins the tail at u = 0.9');
  assert.notDeepEqual(bolts()[1].path, paths[1], 'the sine wave keeps animating after the final hit');
  travel(0.12);
  assert.equal(bolts().length, 1);
  assert.ok(Math.abs(bolts()[0].path[0][0] - 175) <= 10, 'tail moves halfway along the final hop');
